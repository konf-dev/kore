...
| `if` | `(bool then else --)` | Conditional: if bool, run then, else run else |
...
3. **VERIFY AFTER ACTIONS** — Check files exist after creating them
4. **USE `done` WHEN FINISHED** — Signal completion with a message
5. **HANDLE ERRORS** — Check return codes, use `try` for risky operations
